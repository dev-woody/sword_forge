...
export function exchangeByIndex(idx) {
    ...
    for (const [k, v] of Object.entries(rule.in)) {
        const isMat = typeof (st.inv.mats[k]) === "number";
        if (isMat) {
            if ((st.inv.mats[k] ?? 0) < v)
                return { ok: false, reason: "LACK_MAT" };
        }
        else {
            const have = st.inv.swords.filter(sw => sw.name === k).length;
            if (have < v)
                return { ok: false, reason: "LACK_SWORD" };
        }
    }
    // 소모
    useGame.setState(s => {
    ...
    for (const [k, v] of Object.entries(rule.out)) {
        ...
        if (k.startsWith("+")) {
            ...
            if (m) {
                ...
                for (let i = 0; i < v; i++)
                    useGame.getState().addSword({ id: crypto.randomUUID().slice(0, 8), tier: 0, level: lv, name: nm });
                continue;
            }
        }
        // 일반명 아이템을 "레벨0 검"으로 추가(필요 시 개별 로직 분기)
        for (let i = 0; i < v; i++)
            useGame.getState().addSword({ id: crypto.randomUUID().slice(0, 8), tier: 0, level: 0, name: k });
    }
    return { ok: true };
}
