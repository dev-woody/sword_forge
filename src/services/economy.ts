...
function setFailureState(
  args: Omit<FailureState, "dropCollected"> & { dropCollected?: boolean }
) {
  const f: FailureState = { ...args, dropCollected: args.dropCollected ?? false };
...
/** 요구 재료/검 소모 */
function hasAndConsumeRequires(req?: Record<string, number>): boolean {
  if (!req) return true;
  const st = useGame.getState();
  // 1) 보유 확인
  for (const [k, need] of Object.entries(req)) {
    const matQty = st.inv.mats[k as any];
    if (typeof matQty === "number") {
      if ((matQty ?? 0) < need) return false;
    } else {
      if (st.inv.swords.filter((sw) => sw.name === k).length < need) return false;
    }
  }
  // 2) 소모
  useGame.setState((s) => {
    ...
    for (const [k, need] of Object.entries(req)) {
      if (typeof mats[k as any] === "number") mats[k as any] -= need;
      else {
        let left = need;
...
export function enhanceToNext(swordId: string) {
  const st = useGame.getState();
  const sw = st.inv.swords.find((s) => s.id === swordId);
  if (!sw) return { ok: false, reason: "NO_SWORD" };

  const next = sw.level + 1;
  const stage = STAGES[st.mode][next];
  if (!stage || stage.prob === undefined) return { ok: false, reason: "NO_STAGE" };
  ...
  // 요구물
  if (!hasAndConsumeRequires(stage.requires)) return { ok: false, reason: "LACK_REQ" };

  // 방지권은 실패 후 복구에서 사용(사전 소모 안 함)

  // 시도
  useGame.getState().record({ attempts: st.stats.attempts + 1 });
  const success = Math.random() < (stage.prob as number);
  ...
  const protectNeed =
    stage.protect !== "FORBIDDEN" && stage.protect !== "-" ? Number(stage.protect) : null;
  setFailureState({
    swordId: sw.id,
    prevLevel,
    prevName,
...
export function collectFailureDrop() {
  const st = useGame.getState();
  const f = st.failure;
  if (!f || !f.drop || f.dropCollected) return { ok: false, reason: "NO_DROP" };
...
export function restoreFromFailure() {
  const st = useGame.getState();
  const f = st.failure;
  if (!f) return { ok: false, reason: "NO_FAILURE" };
  if (f.protectNeed == null) return { ok: false, reason: "FORBIDDEN" };

  const have = st.inv.tickets.Protect ?? 0;
  if (have < f.protectNeed) return { ok: false, reason: "NO_PROTECT" };
...
export function sellSword(swordId: string) {
  const st = useGame.getState();
  const sw = st.inv.swords.find((s) => s.id === swordId);
  if (!sw) return { ok: false, reason: "NO_SWORD" };
  const stage = STAGES[st.mode][sw.level];
  const price = stage?.sellGold ?? null;
  if (price === null || price === "-") return { ok: false, reason: "NO_PRICE" };

  useGame.getState().addGold(price as number);
