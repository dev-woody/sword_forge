...
const WARP_NAME = { "+9": "타우 스워드", "+13": "불꽃 마검", "+14": "마검 아포피스", "+15": "데몬 배틀 엑스" };
export function buyProtect(count) {
    const st = useGame.getState();
    const price = count === 1 ? SHOP_PRICE[st.mode].PROTECT_1 : SHOP_PRICE[st.mode].PROTECT_3;
    if (!useGame.getState().spendGold(price))
        return { ok: false, reason: "NO_GOLD" };
    useGame.getState().addProtect(count);
    return { ok: true };
}
export function buyWarp(kind) {
    const st = useGame.getState();
